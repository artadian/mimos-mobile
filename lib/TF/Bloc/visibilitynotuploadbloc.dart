...
VisibilityNotUploadBloc() {
  ...
  Future<List<POSMModelTF>> getDataPOSMNotUploadByMaterialID(
    ...
    if (query != null) {
      if (query.isNotEmpty)
        strSQL =
                "SELECT distinct v.userid, v.visittrxid, v.customerno,v.tglkunjungan,c.name as materialname,c.address as typedescription,c.city as statusdescription FROM visittf v INNER JOIN visibility s ON v.customerno = s.customerno AND v.tglkunjungan = s.tglkunjungan AND v.userid = s.userid INNER JOIN customer c ON v.customerno = c.customerno AND v.tglkunjungan = c.tanggalkunjungan AND v.userid = c.userid INNER JOIN materialtf m ON s.materialid = m.materialid where m.materialgroupid is not null and v.getidvisibility <>'-1' and (s.getidvisibilitydetail = '-1' or v.iseditvisibility='Y') and m.materialname  like'%" +
                query +
                " %'  ORDER BY v.visittrxid ASC ";
    } else {
      strSQL =
          "SELECT distinct v.userid, v.visittrxid, v.customerno,v.tglkunjungan,c.name as materialname,c.address as typedescription,c.city as statusdescription FROM visittf v INNER JOIN visibility s ON v.customerno = s.customerno AND v.tglkunjungan = s.tglkunjungan AND v.userid = s.userid INNER JOIN customer c ON v.customerno = c.customerno AND v.tglkunjungan = c.tanggalkunjungan AND v.userid = c.userid INNER JOIN materialtf m ON s.materialid = m.materialid where m.materialgroupid is not null and v.getidvisibility <>'-1' and (s.getidvisibilitydetail = '-1' or v.iseditvisibility='Y') ORDER BY v.visittrxid ASC ";
    }
   // print(strSQL);
