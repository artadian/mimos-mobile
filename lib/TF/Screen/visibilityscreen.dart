...
import 'package:mimos/TF/Screen/visibilityeditscreen.dart';
import 'package:mimos/TF/Screen/visibilityinputscreen.dart';
...
final _dbProvider = DatabaseProvider.dbProvider;
  ...
  goToInput(String customerno, customername, tglkunjungan, userid, priceid,alamat) {
    var root = MaterialPageRoute(
        builder: (context) => new VisibilityInputScreen(
              customerno: customerno.toString(),
              customername: customername.toString(),
              tglkunjungan: tglkunjungan.toString(),
              userid: userid.toString(),
  ...
  Future<List<StockModelTF>> _getAllStock(
    ...
    result = await db.rawQuery(
        "select s.visibilitytrxid as stocktrxid,s.customerno,s.tglkunjungan,s.materialid,m.materialname,s.pac,s.ismaterialdefault,s.iscek from visittf v INNER JOIN visibility s ON s.customerno = v.customerno AND v.userid = s.userid AND v.tglkunjungan = s.tglkunjungan INNER JOIN materialtf m ON s.materialid = m.materialid where s.customerno ='" +
            customerno +
            "' and s.tglkunjungan ='" +
            tglkunjungan +
            "' and v.userid ='" +
            userid +
            "' ORDER BY s.visibilitytrxid Desc");
    ...
    if (snapshot.hasData) {
      return snapshot.data.length != 0
          ...
          : Container(
              ...
              child: Text("Data cek display tidak ada",style: TextStyle(fontSize: 19)),
            ));
    } else {
      return Center(
